...
1. 让 Scenario 2 mock `POST /v1/agent-runs/2/cancel`，记录请求次数并返回成功取消响应。
2. 改用 `.send-btn--stop[aria-label="终止"]` 定位真正运行时按钮。
3. 在点击后断言 cancel 请求发生一次、输入重新可编辑、无“发送失败”。
4. 在现状代码运行该测试，确认它失败于缺少取消请求。
...
1. 为输入停止键增加由父层传入的 `canStop` 状态；SSE 已开启但尚未收到 `stream_start`/`run_id` 时禁用。
2. 在运行页新增 `handleStop()`：已有 current run 时先 await `runCtrl.cancel()`；成功才停止本地 SSE、narration 与 polling，并展示取消反馈。
3. 对取消错误显示真实错误，不改变当前 run 为 cancelled，也不关闭流，保留重试入口。
4. 将两个输入区的 `@stop` 绑定至该处理器，令 Task 1 的 E2E 变绿。

**验收：** 点击输入停止键恰好请求一次 `/cancel`，run 进入 `cancelled`，输入恢复；取消失败不产生假成功。
...
3. 删除运行页 estimate state binding/request handler，并在 E2E 断言输入后不出现预估文案且不请求 estimate API。
...
- `numind-web-v3/e2e/agent-streaming.spec.ts`（或已有最贴近的 Agent 组件测试）

**步骤：**

1. 删除 welcome identity DOM 和对应 CSS，只保留欢迎语。
2. 删除“新内容”文本，调整回到底部按钮为固定圆形；保留 `aria-label="回到底部"`、图标、hover 与 focus。
3. 添加浏览器或组件断言覆盖两处呈现结果及语义名称。

**验收：** 欢迎区不含 emoji/name pill；被打断滚动时仅出现可访问的圆形箭头，点击继续回到最新消息。
