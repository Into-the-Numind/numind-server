...
### 待 S1 与用户澄清的点
1. **时间范围**：单月查看（选月份，默认当月）即可，还是需要**多月历史列表 / 月份区间**？（admin 版是单月 `?month=YYYY-MM`）
2. **空状态**：父账户某月没有任何开通记录时的页面表现（空状态 + 文案）。
3. **入口位置**：放在用户端哪个导航位置？（如「客户管理」附近，或个人中心/账单页）
4. **Booster 加量包**：admin 结算版**完全排除 booster**（子账户自购，不由父账户承担）。父账户对账页是否同样只展示 trial + 月订阅，不展示 booster？（倾向：保持与结算口径一致，排除 booster）
5. **金额口径**：是否展示「本月合计」总额；币种/单位展示（後端 `amount_cents` → 前端 ¥ 元）。
