...
2. **commit 2（feature）** `feat(sop): expose per-step uploaded files in run status response`
   ...
   - `biz/sop/sop.go`：
     ...
     - 新**可测试 seam** `attachNodeFiles(ctx, nodes []CompletedNodeInfo, files []model.SopFile, signImage, signDownload func(context.Context, string, ...) (string,...))`——按 `*NodeID` 分组、判 image-ness、签名（失败/空 objectKey 回退 base url）、回填 `nodes[i].Files`。设计成签名函数注入，便于单测不依赖 COS。
   ...
   - **单测** `replay_files_test.go`：直接测 `attachNodeFiles` + `isImageExt`：
     ...
     - signer 返回 err → 回退 base FileURL；
     - `isImageExt` 大小写/各扩展名/未知扩展名。

**验收**：`go build ./...` 通过；`go test ./internal/numind/biz/sop/...` 通过；`task lint` 退出 0；`GET /sop/runs/:id/status` 的 completed_nodes[] 在有文件时含 files[]（签名 URL）。
...
- `types.ts`：定义 `SopReplayFile`（同构）；`SopNodeRun` 加 `files?: SopReplayFile[]`。为避免 api↔types 循环引用，在 `types.ts` 定义 `SopReplayFile`，`api/sop.ts` 的 `StatusNodeFileInfo` 直接 `=` 复用或重声明一致（实现时择 types.ts 为单一真相，api re-export）。
- `stores/sopRun.ts`：
...
**验收**：`npm run type-check` 退出 0；`npm run lint` 退出 0；`npx vitest run src/views/sop/utils` 通过；store/api 改动不破坏现有类型。
...
- `ReplayInputCard.vue`（只读，props `{ input: string; files?: SopReplayFile[] }`）：
  ...
  - 图片放大复用 `@/components/agent/AgentImagePreview.vue`（props `url`）+ 本地 `previewUrl` ref（或 `useImagePreview`）。
...
- **关键用户路径（S5/S6 验证清单）**：
  ...
  5. 纯文本步骤 / 无任何输入步骤：分别正常显示 / 不报错不出空卡（AC6）。
