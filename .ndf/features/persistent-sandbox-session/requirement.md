...
> **⚠️ 这个对比是"动机参考"，不是"安全论据"（S0 reviewer P1）**：Claude Code / Codex 是**单用户本地工具**——持久的是用户**自己电脑上、自己信任的**文件系统。我们的沙箱是**多租户服务端**，跑的是 **LLM 生成的不可信代码**、为众多远程用户**同时**服务。两者的威胁模型**不在一个量级**。
> 需要在 S1/S2 明确论证的点：持久化**不改变容器隔离边界**（容器仍然是 per-run、一容器只服务一个 run、绝不跨用户共享——这条是硬约束）；但它**确实**让"同一 run 内更早的工具调用产物"在后续调用中可见。在本地开发工具上这无害（用户信任自己的代码）；在多租户服务端，要分析：(a) 长活容器是否扩大单个 run 内被注入代码的可乘之机（如先写 payload 再触发）；(b) 容器存活从秒级变对话级，资源占用/泄漏面如何收敛。**"业界都持久化"只能作为"这条路走得通"的旁证，不能作为"对我们也安全"的证明。**
...
- **Subagent / 任务分解**（用户 Q2 的另一半）——这是比持久沙箱大得多的能力（agent 编排、上下文 fork、并发协调），独立立项
  - **但有依赖约束（S0 reviewer P1）**：业务目标 #3 把持久沙箱定位为 subagent 的前置基础设施。因此 **S1 方案必须验证"per-run 单容器模型"是 subagent-forward 的**——即未来加 subagent 时，这套容器生命周期/池子模型能自然扩展到"一个 run 下多个并发 sub-context 各自的沙箱"，而不是设计成一条到时候要推倒重来的死路。若 S1 评估后认为无法兼顾，必须**显式记录"接受未来 subagent 落地时重设计沙箱模型"这个风险**，不能默默留坑。
