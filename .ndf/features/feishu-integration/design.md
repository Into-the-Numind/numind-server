...
type UserThirdPartyAccount struct {
    ...
    TokenExpiresAt *time.Time // 指针:避免 GORM 零值写成 0001-01-01 被误判过期
...
### 端点契约（请求/响应/errno）
统一走 `core.WriteResponse`（`{code,message,data}`）。
- `POST /v1/feishu/connect`：req 空（userID 从 token）。resp `data:{next_step:"create_app"|"authorize", url:string, state:string}`。
- `GET /v1/feishu/oauth/callback?code=&state=`：**浏览器端点**，成功后 **302 redirect 到前端** `${web}/settings/connections?feishu=connected`（失败 `?feishu=error&reason=`）；不返回 JSON。callback 内部完成 换token→加密 Upsert→`biz.Answer` 恢复。
- `GET /v1/feishu/status`：resp `data:{connected:bool, status:"none"|"active"|"expired", scopes:[]string, app_id:string}`。
- `DELETE /v1/feishu/connection`：resp `data:null`（code 0）。
- **errno（新增 `internal/pkg/errno/feishu.go`）**：`ErrLarkNotConnected` / `ErrLarkReauthRequired` / `ErrLarkCallFailed` / `ErrLarkStateInvalid`。
- **callback 幂等**：run 已不在 `waiting_for_user_choice`（重复回调/刷新）→ 不报错，直接 302 到 success 页；token 已存则跳过 Upsert。

### state 设计（防 CSRF/重放）
`state = base64url( payload + HMAC_SHA256(payload, KEY_STATE) )`，`payload = {user_id, run_id, step, question_text, nonce, exp}`。
- **KEY_STATE 独立配置** `security.feishu_state_key`，**与 §4 的 AES token 密钥分离**（泄露隔离）。
- **nonce 一次性**：生成时写 Redis `feishu:state:<nonce>`（TTL=exp）；callback 验 HMAC+exp 后**校验 nonce 存在并立即删除**（防 exp 窗口内 state 重放）。
- callback 验通过后解出 user_id/run_id，并校验 `run.UserID==user_id`（防跨用户）。
...
- **不新增** `TerminalReason`/`LoopEvent`（CLAUDE.md §6b：经 reviewer 复核实为 `[14]TerminalReason` + `[21]LoopEvent` 编译期不变量）→ 授权暂停复用 `TerminalWaitingForUserChoice` + `LoopEventAskUserPaused`。
- runner 处理 yieldError 的逻辑不依赖工具名（已验证 `runner.go:1213` 纯类型断言），无需改。

### ⚠️ 恢复键机制（P0 修复 — 必须遵守）
`answer.go:108` 强制：`biz.Answer` 的 `Answers` map 每个 key **必须精确等于** `pending_question_json.Questions[i].Question` 文本，否则报 `"question %q was not asked"`，run 永久卡在 `waiting_for_user_choice`。因此 auth 类 yield **不能让 Questions 为空**：

- auth 工具触发 yield 时，`Questions` **必须恰好含一条**：`Question = "<固定提示文，如 飞书授权链接已发送，完成授权后自动恢复>"`（硬编码在 lark 工具里），同时设 `PauseType=auth` + `AuthURL`。
- `state.question_text = Questions[0].Question`（生成 OAuth URL 时编码进 state）。
- callback 恢复时：`biz.Answer(run_id, {state.question_text: {free_text:"已授权"}})` → key 命中 `asked[qText]` → 恢复成功。
- `mergeResumeTranscript` 撑多次暂停（建应用 + 授权两段）。
...
### 边界处理（补 PRD §4 boundary）
- **auth/state 过期**：callback 验 `exp` 失败或 nonce 已消费 → 302 到 `?feishu=error&reason=expired`；agent 侧该 run 仍在 waiting，工具/前端提示「链接已失效，请重新发起连接」（重新发起会生成新 state，旧的自然作废）。
- **用户放弃授权**：run 长期停在 `waiting_for_user_choice` 不报错；用户下次可在连接入口重新发起，或取消该 run（走现有 run 取消路径）。
...
## §13 任务切分预告（S3 细化）
- **后端先行**：crypto 包 → 表+store → **Phase 0 Spike（最先，打掉 bootstrap R1）** → provisioning+oauth+callback → yield 扩展 → client → 3 工具 → trace。
- **bootstrap fallback 双分支**（Spike 后二选一，S3 须各立一个 task，互斥）：`T-bootstrap-native`（native 调 spark/apps 建 app）/ `T-bootstrap-wrapper`（包 lark-cli 藏在 `biz/feishu/provisioner` 接口后，仅建 app 这步；附飞书 ToS 核实）。其余 task 与分支选择解耦。
- **前端后随**：feishu api+store → 连接 UI → 授权卡片（pause_type=auth 渲染）。
- **S5 验证策略**（满足规则 10，S3 由独立 reviewer 审）：方式=gstack 浏览器端到端（连接→建应用→授权→agent 写出 docx），无持久回归代码 → 关键路径补 Go 单测（crypto 加解密、state 签验、token 刷新）。**前置环境**：飞书测试账号 + 在飞书后台把 callback URL 加进重定向白名单 + Spike 产出的测试 app_id/secret + dev 的 `security.feishu_state_key`/`thirdparty_token_key` 注入。
