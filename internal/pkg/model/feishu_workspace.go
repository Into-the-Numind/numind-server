...
type FeishuAuthSession struct {
	...
	UserID              uint           `gorm:"not null;index:idx_feishu_auth_session_user_generation,priority:1" json:"user_id"`
...
type FeishuOperation struct {
	ID                 string         `gorm:"type:char(36);primaryKey" json:"id"`
	UserID             uint           `gorm:"not null;uniqueIndex:uniq_feishu_operation_user_key,priority:1;index:idx_feishu_operation_user_generation,priority:1" json:"user_id"`
