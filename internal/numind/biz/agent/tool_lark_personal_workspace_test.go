...
func TestPlatformToolFactory_LarkPersonalWorkspaceBothOrNoneAndNoLegacyTools(t *testing.T) {
	...
	tools, metadata, err := factory.LoadTools(context.Background())
	require.NoError(t, err)
	require.Len(t, tools, 25, "21 base tools plus exactly four controlled Lark tools")
	require.Len(t, metadata, 25)
	assert.Equal(t, "lark_skill_read", tools[21].Name())
	assert.Equal(t, "lark_inspect", tools[22].Name())
	assert.Equal(t, "lark_connect", tools[23].Name())
	assert.Equal(t, "lark_execute", tools[24].Name())
	assert.Equal(t, "lark_skill_read", metadata[21].ToolName)
	assert.Equal(t, "safe", metadata[21].RiskLevel)
	assert.Equal(t, "飞书", metadata[21].Category)
	assert.Equal(t, "lark_inspect", metadata[22].ToolName)
	assert.Equal(t, "safe", metadata[22].RiskLevel)
	assert.Equal(t, "飞书", metadata[22].Category)
	assert.Equal(t, "lark_connect", metadata[23].ToolName)
	assert.Equal(t, "moderate", metadata[23].RiskLevel)
	assert.Equal(t, "飞书", metadata[23].Category)
	assert.Equal(t, "lark_execute", metadata[24].ToolName)
	assert.Equal(t, "moderate", metadata[24].RiskLevel)
	assert.Equal(t, "飞书", metadata[24].Category)
	assert.Contains(t, metadata[24].Description, "Drive")
