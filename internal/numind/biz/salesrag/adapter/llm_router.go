...
const intentAnalysisPrompt = `你是一个专业的销售意图分析师。分析以下客户发来的消息，完成两个任务：
...
func (r *LLMRouter) AnalyzeIntentV2(ctx context.Context, query string, history []string) (*port.IntentAnalysisResult, error) {
	// 构建历史上下文
	historyStr := "无"
	if len(history) > 0 {
		// 只取最近 5 轮历史
	...
	// 构建 Prompt
	prompt := fmt.Sprintf(intentAnalysisPrompt, historyStr, query)
	...
	if err != nil {
		log.C(ctx).Errorw("LLM intent analysis failed", "error", err)
		// Fallback: 返回默认询问意图
	...
	// 解析 JSON 响应
	// 尝试提取 JSON 部分（可能被包裹在 markdown code block 中）
	jsonStr := extractJSON(resp)

	var result port.IntentAnalysisResult
	if err := json.Unmarshal([]byte(jsonStr), &result); err != nil {
		log.C(ctx).Warnw("Failed to parse intent JSON", "response", resp, "error", err)
		// Fallback
		return &port.IntentAnalysisResult{
			Intent:        port.IntentInquiry,
			SearchQueries: []string{query},
...
func (r *LLMRouter) AnalyzeIntent(ctx context.Context, query string, history []string) (port.IntentType, string, error) {
	result, err := r.AnalyzeIntentV2(ctx, query, history)
	if err != nil {
