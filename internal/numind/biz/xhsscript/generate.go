...
const xhsScriptSystemPrompt = `你是小红书视频口播稿的「爆款结构解剖 + 创作转译」编剧。你擅长把参考视频的叙事骨架、心理触发、情绪逻辑、过渡方式、主题承接和口播节奏拆出来，再迁移到用户自己的创作定位上。
...
func buildGenerationPrompt(profileText string, note *model.XhsScriptNote) string {
	...
	return fmt.Sprintf(`请把下方“小红书爆款视频逐字稿”转译成一篇适合“我的创作定位”的原创口播稿。
...
如果原文某个结构、案例、情绪强度不适合我的创作定位，可以保留它的功能，但必须换成更适合当前主题、账号或产品服务的表达方式。不要为了追求 1:1 仿写而牺牲可信度和自然度。
...
迁移到我的创作定位时，不要直接复用原文概念，不要把原文关键词机械替换成我的主题关键词。你要为当前账号、主题或产品服务重新提炼一个能支撑整篇文稿的核心概念。这个概念必须能同时连接目标人群、内容价值和最终行动理由。
...
从“我的创作定位”中提取本次创作的核心论点、目标人群、内容价值、可信证据和行动号召。

如果创作定位信息充足，要优先使用其中的真实信息。

如果创作定位信息不足，可以使用目标人群常见痛点、内容场景和合理经验判断，但不能编造资质、价格、案例结果、收益数字、客户细节、调查数据或不可验证承诺。
...
新稿对应段落要尽量承担相同功能，但内容必须完全服务于我的创作定位。

如果原文某个段落无法自然迁移，可以保留它的说服功能，重新组织为更适合当前创作定位的场景、观点或过渡。
...
主题、观点或产品服务的承接要自然，必须从前文观点里长出来，不能最后硬插广告。
