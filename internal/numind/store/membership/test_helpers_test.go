// Package membership_test provides shared test helpers for the membership store
// package. Placed in a _test.go file (external test package) so it doesn't
// ship in production binaries — following the billing_test.go pattern from the
...
// newTestDB creates an isolated in-memory SQLite DB for membership store tests.
// Uses raw DDL (not AutoMigrate) to avoid SQLite ENUM incompatibility with
// GORM tag `type:enum(...)` on MySQL models.
