...
-- 把请求路由到默认服务 (qwen-turbo). 管理端可后续切换到 qwen-plus 或其它兼容 LLM.
--
-- 选 qwen-turbo 理由 (spec §3.2 决策):
...
--   default_service_id link to ai_service.id WHERE model_key = 'qwen-turbo'
...
-- 若 qwen-turbo 在目标环境 ai_service 表中不存在, SELECT 返回空, INSERT IGNORE 跳过 — sanitize 调用会
-- surface ErrSanitizeUnavailable (T7 errno.Marketplace.SanitizeUnavailable, HTTP 503), 这是预期 graceful
-- degradation. 部署前 verify: SELECT id FROM ai_service WHERE model_key = 'qwen-turbo';
