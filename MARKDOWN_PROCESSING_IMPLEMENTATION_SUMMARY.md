...
基于用户需求，将卡册创建功能从JSON格式改为markdown格式，并使用配置文件中的AI提示词进行处理。

## 主要修改

### 1. 启用Markdown处理模式
在`config_local.yaml`中启用markdown处理：
```yaml
book:
    use_markdown_processing: true  # 启用Markdown处理模式
```
...
通过这次修改，我们成功地将卡册创建功能从JSON格式改为markdown格式，并使用配置文件中的AI提示词进行处理。主要改进包括：

1. **更自然的格式**：使用markdown格式而不是JSON，更符合内容处理的自然流程
...
这个实现完全基于现有逻辑进行修改，没有重写核心功能，而是通过增强现有方法来实现新的需求。
