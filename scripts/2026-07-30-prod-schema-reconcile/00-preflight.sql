...
SELECT
  ...
  CONCAT(
    'present=', COUNT(*),
    ',metadata_exact=', COALESCE(SUM(
      CASE COLUMN_NAME
        WHEN 'parsed_content' THEN
          COLUMN_DEFAULT IS NULL AND EXTRA = ''
          AND CHARACTER_SET_NAME = 'utf8mb4' AND COLLATION_NAME = 'utf8mb4_unicode_ci'
        WHEN 'parsed_content_sha256' THEN
          EXTRA = '' AND CHARACTER_SET_NAME = 'utf8mb4'
          AND COLLATION_NAME = 'utf8mb4_unicode_ci'
        WHEN 'parsed_content_byte_size' THEN
          EXTRA = '' AND CHARACTER_SET_NAME IS NULL AND COLLATION_NAME IS NULL
        WHEN 'parsed_page_count' THEN
          EXTRA = '' AND CHARACTER_SET_NAME IS NULL AND COLLATION_NAME IS NULL
        WHEN 'parsed_at' THEN
          COLUMN_DEFAULT IS NULL AND EXTRA = ''
  ...
  'all present columns have exact final-or-legacy metadata'
FROM information_schema.COLUMNS
WHERE TABLE_SCHEMA = DATABASE()
  AND TABLE_NAME = 'agent_attachment'
  AND COLUMN_NAME IN (
