...
# 获取管理员token
get_admin_token() {
    print_info "尝试获取管理员token..."
    
    RESPONSE=$(curl -s -X POST "${BASE_URL}/v1/admin/login" \
    ...
    if [ "$TOKEN" != "null" ] && [ -n "$TOKEN" ]; then
        print_success "获取管理员token成功"
        return 0
    else
        print_warning "管理员登录失败，尝试微信登录..."
        return 1
    fi
}
