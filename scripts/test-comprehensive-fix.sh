...
# 测试2: 创建综合测试程序
echo "创建综合测试程序..."
cat > test_comprehensive_fix.go << 'EOF'
package main
...
func main() {
	...
	coverCard2 := &model.CardM{
		...
		ProcessedText: func() string {
			coverElements := []map[string]interface{}{
				{"type": "title", "content": "魅力的本质"},
				{"type": "background", "content": "/Users/neozhang/go/src/github.com/Into-the-Numind/numind-server/res/template/t3.png"},
			}
			if b, err := json.Marshal(coverElements); err == nil {
				return string(b)
			}
	...
	// 验证分页逻辑
	fmt.Println("\n=== 验证分页逻辑 ===")
	if len(paginatedContent.Cards) > 0 {
		for i, card := range paginatedContent.Cards {
			...
			for _, element := range card.Elements {
				...
				case pagination.ElementTypeTitle:
					elementHeight = 64 + 30 + 30 // 字体大小 + 上边距 + 下边距
				case pagination.ElementTypeSubtitle:
					elementHeight = 48 + 30 + 30
				case pagination.ElementTypeBody:
					elementHeight = 36 + 30 + 30
			...
			availableHeight := config.Card.Height - config.Card.Padding.Top - config.Card.Padding.Bottom
			if totalHeight <= availableHeight {
				fmt.Printf("✅ 卡片 %d 高度正常: %d <= %d\n", i+1, totalHeight, availableHeight)
			} else {
				fmt.Printf("❌ 卡片 %d 高度超出: %d > %d\n", i+1, totalHeight, availableHeight)
			}
		}
	}
...
# 测试3: 运行测试程序
echo "运行测试程序..."
go run test_comprehensive_fix.go

# 测试4: 清理测试文件
echo "清理测试文件..."
rm -f test_comprehensive_fix.go
